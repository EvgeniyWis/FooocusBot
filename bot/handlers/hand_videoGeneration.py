...
async def handle_prompt_for_videoGenerationFromImage(message: types.Message, state: FSMContext):
    ...
    # Сохраняем промпт в стейт, где ключом будет file_id изображения
    if "prompts_for_videoGenerationFromImage" not in stateData:
        await state.update_data(prompts_for_videoGenerationFromImage={image_file_id: prompt})
    else:
        stateData["prompts_for_videoGenerationFromImage"][image_file_id] = prompt
