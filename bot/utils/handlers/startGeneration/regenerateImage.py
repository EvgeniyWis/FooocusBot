...
async def regenerateImage(model_name: str, call: types.CallbackQuery, state: FSMContext, setting_number: str):
    ...
    try:
        return await generateImageBlock(json, model_name, call.message.message_id, 
        state, user_id, setting_number, is_test_generation, False, chat_id=call.message.chat.id)
    finally:
