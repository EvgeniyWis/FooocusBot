...
async def sendImageBlock(state: FSMContext, media_group: list, model_name: str,
    ...
    try:
        ...
        try:
            if isinstance(e, TelegramRetryAfter):
                await bot.send_message(chat_id=user_id, text=f"Превышен лимит отправки сообщений. Пожалуйста, подождите {e.retry_after} секунд и попробуйте снова.")
            else:
                await bot.send_message(chat_id=user_id, text="Произошла ошибка при отправке изображений, но продолжаем работу...")
        except:
            pass
    ...
    try:
        ...
        try:
            await bot.send_message(chat_id=user_id, text=text.SELECT_IMAGE_TEXT.format(model_name, model_name_index) if not is_test_generation else text.SELECT_TEST_IMAGE_TEXT.format(setting_number),
            ...
            try:
                await bot.send_message(chat_id=user_id, text="Произошла ошибка при отправке клавиатуры...")
            except:
                pass
