...
async def getNextModel(current_model: str, setting_number: str, state: FSMContext):
    if setting_number == "all":
        ...
        current_model_is_last_in_setting = current_model == dataArrayBySettingNumber[-1]["model_name"]

        logger.info(f"Текущая настройка: {current_setting_number}. Является ли модель {current_model} последней в настройке: {current_model_is_last_in_setting}")
