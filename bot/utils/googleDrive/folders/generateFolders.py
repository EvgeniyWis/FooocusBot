from utils.googleDrive.folders.createFolder import createFolder
...
async def generateFolders(model_names: list[str]) -> None:
    ...
    for model_name in model_names:
        ...
        logger.info(f"Папка для модели {model_name} создана: {id} по ссылке: {link}")
        model_folders_ids.append(id)

    # Создаём словарь с именем модели и id
    model_folders_dict = {model_name: model_folder_id for model_name, model_folder_id in zip(model_names, model_folders_ids)}

    # Генерируем папки "picture" и "videos" в корневых папках моделей
    picture_folders = []
    videos_folders = []
    for model_name, model_folder_id in model_folders_dict.items():
        picture_folder = await createFolder("picture", parent_folder_id=model_folder_id)
        logger.info(f'Папка "picture" для модели {model_name} создана: {picture_folder}')
        
        videos_folder = await createFolder("videos", parent_folder_id=model_folder_id)
        logger.info(f'Папка "videos" для модели {model_name} создана: {videos_folder}')
